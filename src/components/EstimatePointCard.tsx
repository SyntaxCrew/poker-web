export default function EstimatePointCard(props: {estimatePoint: number | '?' | '!', currentPoint?: number, onSelect: (estimatePoint?: number) => void}) {
    ...
    return (
        <div
            className={"w-12 h-20 rounded-md flex items-center cursor-pointer ease-in duration-100 " + (props.currentPoint === point ? 'bg-blue-600 text-white -translate-y-2' : 'bg-white text-blue-600 border-2 border-blue-600 hover:bg-blue-200 hover:-translate-y-2')}
